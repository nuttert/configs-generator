 ...
 * </pre>
 */
public class CheckParsing implements Mustache.Lambda {
    private CodegenConfig generator = null;
    private Boolean escapeParam = false;
    private Boolean lowercaseFirstLetter = true;

    public CheckParsing(boolean lowercaseFirstLetter) {
      this.lowercaseFirstLetter = lowercaseFirstLetter;
    }

    public CheckParsing() {}

    public CheckParsing generator(final CodegenConfig generator) {
        this.generator = generator;
        return this;
    }

    public CheckParsing escapeAsParamName(final Boolean escape) {
    ...
    public void execute(Template.Fragment fragment, Writer writer) throws IOException {
        String text = fragment.execute().replace("&gt;",">");
        text = text.replace("&lt;","<");
        text = text.replace("&quot;","\"");
        text = text.replace("&amp;","&");
        text = text.replace("&#x3D;","=");
