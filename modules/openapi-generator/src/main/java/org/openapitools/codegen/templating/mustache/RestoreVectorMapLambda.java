 ...
 * </pre>
 */
public class RestoreVectorMapLambda implements Mustache.Lambda {
    private CodegenConfig generator = null;
    private Boolean escapeParam = false;
    private Boolean lowercaseFirstLetter = true;

    public RestoreVectorMapLambda(boolean lowercaseFirstLetter) {
      this.lowercaseFirstLetter = lowercaseFirstLetter;
    }

    public RestoreVectorMapLambda() {}

    public RestoreVectorMapLambda generator(final CodegenConfig generator) {
        this.generator = generator;
        return this;
    }

    public RestoreVectorMapLambda escapeAsParamName(final Boolean escape) {
    ...
    public void execute(Template.Fragment fragment, Writer writer) throws IOException {
        ...
        if(text.contains("std::vector>") || text.equals("std::vector")){
            text = text.replaceFirst("std::vector","std::vector<std::string>");

        }
        if(text.contains("std::map>") || text.equals("std::map")){
            text = text.replaceFirst("std::map","std::map<std::string, std::string>");
    }
        if (generator != null) {
