 ...
 * </pre>
 */
public class OptionalLambda implements Mustache.Lambda {
    private CodegenConfig generator = null;
    private Boolean escapeParam = false;
    private Boolean lowercaseFirstLetter = true;

    public OptionalLambda(boolean lowercaseFirstLetter) {
      this.lowercaseFirstLetter = lowercaseFirstLetter;
    }

    public OptionalLambda() {}

    public OptionalLambda generator(final CodegenConfig generator) {
        this.generator = generator;
        return this;
    }

    public OptionalLambda escapeAsParamName(final Boolean escape) {
    ...
    public void execute(Template.Fragment fragment, Writer writer) throws IOException {
        String text = fragment.execute().indexOf("shared_ptr") == -1 ?  "std::optional<"+fragment.execute()+">" : fragment.execute();
